...
export function createExternalTooltipHandler(getActiveChartData, getSelectedMetric, getQualityDelta) {
  return ({ chart, tooltip }) => {
    ...
    const ARROW_UP = `<svg width="8" height="8" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="performance-tooltip-arrow"><path d="M12 19V5" /><path d="m5 12 7-7 7 7" /></svg>`;
    const ARROW_DOWN = `<svg width="8" height="8" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="performance-tooltip-arrow"><path d="M12 5v14" /><path d="m19 12-7 7-7-7" /></svg>`;
    ...
    const changeMarkup = isRank ? (change > 0 ? ARROW_UP : change < 0 ? ARROW_DOWN : '') : '';
